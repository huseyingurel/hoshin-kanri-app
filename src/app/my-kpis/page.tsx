...
import { BarChart2, Target, PlusCircle } from "lucide-react";
import Link from "next/link";
import { getSessionOrRedirect } from "@/lib/session";
import { personalKpiScopeFilter, type UserScope } from "@/lib/dataScope";
...
export default async function MyKpisPage() {
  ...
  const kpis = await prisma.kPI.findMany({
    where: personalKpiScopeFilter(scope),
    include: {
    ...
    <div className="flex flex-col gap-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-zinc-400 mt-2">Sorumlu olduğunuz metriklerin Bowling Chart görünümü ve sapma takibi.</p>
        </div>
        <Link href="/data-entry">
          <Button className="bg-blue-600 hover:bg-blue-700 text-white flex gap-2">
            <PlusCircle size={16} /> Yeni Veri Girişi
          </Button>
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {kpis.length === 0 && (
          <p className="text-zinc-500 text-center py-12">Size atanmış veya departmanınıza bağlı KPI bulunmuyor.</p>
        )}
      </div>
    </div>
  );
